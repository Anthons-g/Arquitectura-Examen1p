...
CREATE TABLE children (
  ...
  privacy_settings JSONB DEFAULT '{"share_with_specialists": true, "share_progress_reports": true, "allow_photo_sharing": false, "data_retention_months": 36}',-- Erroe json, escribirlo en una sola linea
...
CREATE TABLE daily_logs (
  ...
  intensity_level TEXT REFERENCES intensity_levels(level) DEFAULT 'medium',
...
CREATE TABLE audit_logs (
  ...
  user_agent TEXT,
  session_id TEXT,
  risk_level TEXT REFERENCES risk_levels(level) DEFAULT 'low',
  created_at TIMESTAMPTZ DEFAULT NOW()
);
